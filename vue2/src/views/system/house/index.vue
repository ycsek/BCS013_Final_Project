<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="楼号/户型" prop="huxing">
        <el-input
          v-model="queryParams.huxing"
          placeholder="请输入楼号/户型"
          clearable
    ...
    <el-table v-loading="loading" :data="houseList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="特色" align="center" prop="characteristic" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="楼号/户型" prop="huxing">
          <el-input v-model="form.huxing" placeholder="请输入楼号/户型" />
        </el-form-item>
        ...
        <el-form-item label="特色" prop="characteristic">
          <el-input v-model="form.characteristic" placeholder="请输入特色" />
        </el-form-item>
        <el-form-item label="状态" prop="status">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        huxing: [
          { required: true, message: "楼号/户型不能为空", trigger: "blur" }
        ],
        price: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除租房发布编号为"' + ids + '"的数据项？').then(function() {
        return delHouse(ids);
      }).then(() => {
        this.getList();
